...
export const IsOrderQueryParamList = (
  property: string,
  orderFieldEnumType: any,
  validationOptions?: ValidationOptions,
) => {
  return function (object, propertyName: string) {
    registerDecorator({
      ...
      validator: {
        validate(value: any) {
          const listOrderValid = value
            .split(',')
            .filter((x) => x.split(':').length === 2)
            .map((x) => {
              const [field, orderDirection] = x.split(':');
              return isValidOrder(field, orderDirection, orderFieldEnumType);
            });
          return listOrderValid.every((x) => x);
        },
      },
    });
  };
